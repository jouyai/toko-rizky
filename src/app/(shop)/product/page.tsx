...
function ProductContent() {
  ...
  const filteredProducts = products
    .filter((p) => {
      ...
      // Check if query matches category OR gender
      return (
        p.category?.toLowerCase() === searchCat ||
        p.gender?.toLowerCase() === searchCat
      );
    })
          ...
          <div className="sticky top-24 space-y-6 bg-white p-6 rounded-xl border border-slate-100 shadow-sm">
            <div className="flex items-center justify-between">
              ...
              <button
                ...
                className="text-xs text-amber-500 font-bold hover:underline uppercase tracking-widest"
              >
                Clear all
              </button>
            </div>
              ...
              <div className="space-y-2">
                <label className="flex items-center gap-3 cursor-pointer group">
                  <input
                    ...
                    className="rounded-full border-slate-300 text-amber-500 focus:ring-amber-500"
                  />
                  <span className={`text-sm group-hover:text-amber-500 transition-colors ${selectedCategory === '' ? 'text-amber-500 font-medium' : 'text-slate-600'}`}>
                    Semua
                  </span>
                </label>
                {categories.map((cat) => (
                  <label key={cat.id} className="flex items-center gap-3 cursor-pointer group">
                    <input
                      ...
                      className="rounded-full border-slate-300 text-amber-500 focus:ring-amber-500"
                    />
                    <span className={`text-sm group-hover:text-amber-500 transition-colors ${selectedCategory === cat.name ? 'text-amber-500 font-medium' : 'text-slate-600'}`}>
                      {cat.name}
                    </span>
                  </label>
                ))}
              ...
              <div className="flex flex-wrap gap-2">
                {['S', 'M', 'L', 'XL', 'XXL'].map((size) => (
                  <button
                    ...
                    className={`w-10 h-10 flex items-center justify-center border rounded text-xs font-bold transition-all ${selectedSizes.includes(size)
                      ? 'bg-slate-900 border-slate-900 text-white'
                      : 'border-slate-200 text-slate-600 hover:border-amber-500 hover:text-amber-500'
                      }`}
                  >
                    {size}
                  </button>
                ...
                {[
                  ...
                  <label key={option.label} className="flex items-center gap-3 cursor-pointer group">
                    <input
                      ...
                      onChange={() => { setPriceRange(option.range); setCurrentPage(1); }}
                      className="rounded-full border-slate-300 text-amber-500 focus:ring-amber-500"
                    />
                    <span className="text-sm text-slate-600 group-hover:text-amber-500 transition-colors">
                      {option.label}
                    </span>
                  </label>
                ))}
            ...
            <button
              ...
              className="lg:hidden w-full py-3 bg-slate-900 text-white font-bold rounded-lg uppercase tracking-widest text-xs"
            >
              Terapkan Filter
            </button>
          </div>
        ...
        <section className="flex-1">
          ...
          {loading ? (
            ...
            <div className="text-center py-20 bg-white rounded-xl border border-slate-100">
              ...
              <button
                onClick={clearFilters}
                className="mt-4 px-6 py-2 bg-slate-900 text-white text-xs font-bold uppercase tracking-widest hover:bg-amber-500 transition-colors rounded-lg"
              >
                Reset Filter
              </button>
            </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 lg:gap-6">
              {paginatedProducts.map((product) => (
                <Link
                  ...
                  className="group bg-white rounded-xl border border-slate-100 overflow-hidden shadow-sm hover:shadow-lg transition-all duration-300"
                >
                  {/* Product Image */}
                  <div className="relative aspect-[3/4] overflow-hidden bg-slate-100">
                    ...
                    <button
                      ...
                      className="absolute top-3 right-3 p-2 bg-white/80 backdrop-blur rounded-full text-slate-900 hover:text-red-500 transition-colors shadow-sm opacity-0 group-hover:opacity-100"
                    >
                    ...
                    {/* Stock Badge */}
                    {product.stock === 0 && (
                      <span className="absolute top-3 left-3 bg-slate-900 text-white text-[10px] font-bold px-2 py-1 rounded uppercase">
                        Habis
                      </span>
                    )}
                  </div>
            ...
            <div className="mt-12 flex flex-col items-center gap-6">
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="w-10 h-10 flex items-center justify-center rounded-lg bg-white border border-slate-200 text-slate-500 hover:text-amber-500 hover:border-amber-500 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                >
                ...
                {Array.from({ length: Math.min(5, totalPages) }, (_, i) => {
                  ...
                  return (
                    <button
                      ...
                      className={`w-10 h-10 flex items-center justify-center rounded-lg font-bold text-sm transition-all ${currentPage === pageNum
                        ? 'bg-slate-900 text-white shadow-md'
                        : 'bg-white border border-slate-200 text-slate-600 hover:text-amber-500 hover:border-amber-500'
                        }`}
                    >
                      {pageNum}
                    </button>
